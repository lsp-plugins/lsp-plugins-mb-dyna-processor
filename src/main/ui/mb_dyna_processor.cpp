...
namespace lsp
{
    namespace plugui
    {
        ...
        {
            return new mb_dyna_processor_ui(meta);
        }
        ...
        template <class T>
        T *mb_dyna_processor_ui::find_split_widget(const char *fmt, const char *base, size_t id)
        {
            char widget_id[64];
        ...
        mb_dyna_processor_ui::split_t *mb_dyna_processor_ui::find_split_by_port(ui::IPort *port)
        {
        ...
        mb_dyna_processor_ui::mb_dyna_processor_ui(const meta::plugin_t *meta): ui::Module(meta)
        {
            fmtStrings      = fmt_strings;
            if (!strcmp(meta->uid, meta::mb_dyna_processor_lr.uid))
            {
                fmtStrings      = fmt_strings_lr;
            }
            else if (!strcmp(meta->uid, meta::mb_dyna_processor_ms.uid))
            {
                fmtStrings      = fmt_strings_ms;
            }
        }

        mb_dyna_processor_ui::~mb_dyna_processor_ui()
        {
        }

        status_t mb_dyna_processor_ui::slot_split_mouse_in(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_dyna_processor_ui *ui = static_cast<mb_dyna_processor_ui *>(ptr);
            if (ui == NULL)
        ...
        status_t mb_dyna_processor_ui::slot_split_mouse_out(tk::Widget *sender, void *ptr, void *data)
        {
            // Fetch parameters
            mb_dyna_processor_ui *ui = static_cast<mb_dyna_processor_ui *>(ptr);
            if (ui == NULL)
        ...
        ui::IPort *mb_dyna_processor_ui::find_port(const char *fmt, const char *base, size_t id)
        {
            char port_id[32];
        ...
        mb_dyna_processor_ui::split_t *mb_dyna_processor_ui::find_split_by_widget(tk::Widget *widget)
        {
        ...
        void mb_dyna_processor_ui::on_split_mouse_in(split_t *s)
        {
            if (s->wNote != NULL)
            {
        ...
        void mb_dyna_processor_ui::on_split_mouse_out()
        {
            for (size_t i=0, n=vSplits.size(); i<n; ++i)
            {
                split_t *d = vSplits.uget(i);
        ...
        void mb_dyna_processor_ui::add_splits()
        {
            size_t channel      = 0;

            for (const char **fmt = fmtStrings; *fmt != NULL; ++fmt, ++channel)
            {
                for (size_t port_id=1; port_id<meta::mb_dyna_processor::BANDS_MAX; ++port_id)
                {
                    split_t s;
        ...
        ssize_t mb_dyna_processor_ui::compare_splits_by_freq(const split_t *a, const split_t *b)
        {
            if (a->fFreq < b->fFreq)
                return -1;
        ...
        void mb_dyna_processor_ui::resort_active_splits()
        {
            vActiveSplits.clear();
        ...
        void mb_dyna_processor_ui::update_split_note_text(split_t *s)
        {
            // Get the frequency
            ...
            // Update the note name displayed in the text
            {
                ...
                if (text.ends_with_ascii("m"))
                    lc_string.set("lists.mb_dyna_processor.splits.index.mid_id");
                else if (text.ends_with_ascii("s"))
                    lc_string.set("lists.mb_dyna_processor.splits.index.side_id");
                else if (text.ends_with_ascii("l"))
                    lc_string.set("lists.mb_dyna_processor.splits.index.left_id");
                else if (text.ends_with_ascii("r"))
                    lc_string.set("lists.mb_dyna_processor.splits.index.right_id");
                else
                    lc_string.set("lists.mb_dyna_processor.splits.index.split_id");

                lc_string.params()->set_int("id", (vSplits.index_of(s) % (meta::mb_dyna_processor::BANDS_MAX-1))+1);
                ...
                if (note_full != dspu::NOTE_OUT_OF_RANGE)
                {
                    ...
                    s->wNote->text()->set("lists.mb_dyna_processor.notes.full", &params);
                }
                else
                    s->wNote->text()->set("lists.mb_dyna_processor.notes.unknown", &params);
            }
        }

        status_t mb_dyna_processor_ui::post_init()
        {
        ...
        void mb_dyna_processor_ui::notify(ui::IPort *port, size_t flags)
        {
        ...
        void mb_dyna_processor_ui::toggle_active_split_fequency(split_t *initiator)
        {
