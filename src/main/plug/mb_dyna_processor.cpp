...
namespace lsp
{
    namespace plugins
    {
        ...
        void mb_dyna_processor::init(plug::IWrapper *wrapper, plug::IPort **ports)
        {
            ...
            // Initialize analyzer
            size_t an_cid       = 0;
            if (!sAnalyzer.init(2*channels, meta::mb_dyna_processor::FFT_RANK,
                                MAX_SAMPLE_RATE, meta::mb_dyna_processor::REFRESH_RATE))
                return;
            ...
            size_t to_alloc =
                ...
                (
                    ...
                    MBDP_BUFFER_SIZE * sizeof(float) * 2 + // vInAnalyze + vOutAnalyze for each channel
                    // Band buffers
                    (
            ...
            ptr            += meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float);
            vCurve          = reinterpret_cast<float *>(ptr);
            ptr            += meta::mb_dyna_processor::CURVE_MESH_SIZE * sizeof(float);
            vIndexes        = reinterpret_cast<uint32_t *>(ptr);
            ptr            += meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(uint32_t);
            vSc[0]          = reinterpret_cast<float *>(ptr);
            ptr            += MBDP_BUFFER_SIZE * sizeof(float);
            if (channels > 1)
            {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    ptr            += meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float) * 2;
            ...
            for (size_t i=0; i<channels; ++i)
            {
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    else
                    {
                        ...
                        b->pAttackTime[0]   = trace_port(ports[port_id++]);
                        b->pReleaseTime[0]  = trace_port(ports[port_id++]);

                        for (size_t j=0; j<meta::mb_dyna_processor::DOTS; ++j)
                        {
                            b->pDotOn[j]        = trace_port(ports[port_id++]);
                            b->pThreshold[j]    = trace_port(ports[port_id++]);
                            b->pGain[j]         = trace_port(ports[port_id++]);
                            b->pKnee[j]         = trace_port(ports[port_id++]);
                            b->pAttackOn[j]     = trace_port(ports[port_id++]);
                            b->pAttackLvl[j]    = trace_port(ports[port_id++]);
                            b->pAttackTime[j+1] = trace_port(ports[port_id++]);
                            b->pReleaseOn[j]    = trace_port(ports[port_id++]);
                            b->pReleaseLvl[j]   = trace_port(ports[port_id++]);
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    if (b->nSync & S_EQ_CURVE)
                    {
                        ...
                        if ((mesh != NULL) && (mesh->isEmpty()))
                        {
                            ...
                            dsp::mul_k3(&mesh->pvData[1][1], b->vSc, b->fScPreamp, meta::mb_dyna_processor::MESH_POINTS);
