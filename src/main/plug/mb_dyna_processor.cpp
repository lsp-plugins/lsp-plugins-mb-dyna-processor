    ...
    namespace plugins
    {
        ...
        void mb_dyna_processor::init(plug::IWrapper *wrapper, plug::IPort **ports)
        {
            ...
            size_t to_alloc =
                ...
                meta::mb_dyna_processor::CURVE_MESH_SIZE * sizeof(float) + // Curve
                meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float) + // vFreqs array
                meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(uint32_t) + // vIndexes array
                MBDP_BUFFER_SIZE * sizeof(float) + // Global vBuffer for band signal processing
                ...
                (
                    ...
                    // Band buffers
                    (
                        MBDP_BUFFER_SIZE * sizeof(float) + // vBuffer of each band
                        MBDP_BUFFER_SIZE * sizeof(float) + // vVCA of each band
                        meta::mb_dyna_processor::FFT_MESH_POINTS * 2 * sizeof(float) + // vSc transfer function for each band
                        meta::mb_dyna_processor::FFT_MESH_POINTS * 2 * sizeof(float) // vTr transfer function for each band
                    ) * meta::mb_dyna_processor::BANDS_MAX
                ) * channels;
            ...
            vFreqs          = advance_ptr_bytes<float>(ptr, meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float));
            vCurve          = advance_ptr_bytes<float>(ptr, meta::mb_dyna_processor::CURVE_MESH_SIZE * sizeof(float));
            vIndexes        = advance_ptr_bytes<uint32_t>(ptr, meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(uint32_t));
            vSc[0]          = advance_ptr_bytes<float>(ptr, MBDP_BUFFER_SIZE * sizeof(float));
            vSc[1]          = (channels > 1) ? advance_ptr_bytes<float>(ptr, MBDP_BUFFER_SIZE * sizeof(float)) : NULL;
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                c->sDryEq.init(meta::mb_dyna_processor::BANDS_MAX-1, 0);
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    if (!b->sSC.init(channels, meta::mb_dyna_processor::REACTIVITY_MAX))
                        return;
                    ...
                    b->vBuffer      = advance_ptr_bytes<float>(ptr, MBDP_BUFFER_SIZE * sizeof(float));
                    b->vVCA         = advance_ptr_bytes<float>(ptr, MBDP_BUFFER_SIZE * sizeof(float));
                    b->vSc          = advance_ptr_bytes<float>(ptr, meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float) * 2);
                    b->vTr          = advance_ptr_bytes<float>(ptr, meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float) * 2);
                    ...
                    b->bEnabled     = j < meta::mb_dyna_processor::BANDS_DFL;
                    b->bCustHCF     = false;
                    b->bCustLCF     = false;
                    b->bMute        = false;
                    b->bSolo        = false;
                    ...
                    for (size_t j=0; j<meta::mb_dyna_processor::DOTS; ++j)
                    {
                        b->pDotOn[j]        = NULL;
                        b->pThreshold[j]    = NULL;
                        b->pGain[j]         = NULL;
                        ...
                        b->pAttackLvl[j]    = NULL;
                        b->pReleaseOn[j]    = NULL;
                        b->pReleaseLvl[j]   = NULL;
                    }
                    for (size_t j=0; j<meta::mb_dyna_processor::RANGES; ++j)
                    {
                        b->pAttackTime[j]   = NULL;
                        b->pReleaseTime[j]  = NULL;
                    }
                ...
                // Initialize split
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX-1; ++j)
                {
            ...
            // Dynamic Processor bands
            lsp_trace("Binding dynamic processor bands");
            for (size_t i=0; i<channels; ++i)
            {
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    if ((i > 0) && (nMode == MBDP_STEREO))
                    {
                        dyna_band_t *sb     = &vChannels[0].vBands[j];
                        ...
                        for (size_t j=0; j<meta::mb_dyna_processor::DOTS; ++j)
                        {
                            b->pDotOn[j]        = sb->pDotOn[j];
                            b->pThreshold[j]    = sb->pThreshold[j];
                            b->pGain[j]         = sb->pGain[j];
                            b->pKnee[j]         = sb->pKnee[j];
                            b->pAttackOn[j]     = sb->pAttackOn[j];
                            b->pAttackLvl[j]    = sb->pAttackLvl[j];
                            b->pReleaseOn[j]    = sb->pReleaseOn[j];
                            b->pReleaseLvl[j]   = sb->pReleaseLvl[j];
                        }
                        for (size_t j=0; j<meta::mb_dyna_processor::RANGES; ++j)
                        {
                            b->pAttackTime[j]   = sb->pAttackTime[j];
                            b->pReleaseTime[j]  = sb->pReleaseTime[j];
                        }
                        b->pHold        = sb->pHold;

                        b->pLowRatio    = sb->pLowRatio;
                        b->pHighRatio   = sb->pHighRatio;
                    ...
                    else
                    {
                        ...
                        BIND_PORT(b->pAttackTime[0]);
                        BIND_PORT(b->pReleaseTime[0]);

                        for (size_t j=0; j<meta::mb_dyna_processor::DOTS; ++j)
                        {
                            BIND_PORT(b->pDotOn[j]);
                            BIND_PORT(b->pThreshold[j]);
                            BIND_PORT(b->pGain[j]);
                            BIND_PORT(b->pKnee[j]);
                            BIND_PORT(b->pAttackOn[j]);
                            BIND_PORT(b->pAttackLvl[j]);
                            BIND_PORT(b->pAttackTime[j+1]);
                            BIND_PORT(b->pReleaseOn[j]);
                            BIND_PORT(b->pReleaseLvl[j]);
                            BIND_PORT(b->pReleaseTime[j+1]);
                        }
                        BIND_PORT(b->pHold);

                        BIND_PORT(b->pLowRatio);
                        BIND_PORT(b->pHighRatio);
                        BIND_PORT(b->pMakeup);

                        // Skip meters visibility controls and hue
                        SKIP_PORT("Meters visibility");
                        SKIP_PORT("Hue");

                        BIND_PORT(b->pFreqEnd);
                        BIND_PORT(b->pModelGraph);
            ...
            float delta = (meta::mb_dyna_processor::CURVE_DB_MAX - meta::mb_dyna_processor::CURVE_DB_MIN) / (meta::mb_dyna_processor::CURVE_MESH_SIZE-1);
            for (size_t i=0; i<meta::mb_dyna_processor::CURVE_MESH_SIZE; ++i)
                vCurve[i]   = dspu::db_to_gain(meta::mb_dyna_processor::CURVE_DB_MIN + delta * i);
        }

        bool mb_dyna_processor::compare_bands_for_sort(const dyna_band_t *b1, const dyna_band_t *b2)
        {
        ...
        void mb_dyna_processor::ui_deactivated()
        {
            sAnalyzer.set_activity(false);
        }

        uint32_t mb_dyna_processor::decode_sidechain_type(uint32_t sc) const
        {
            if (bSidechain)
            {
                switch (sc)
        ...
        void mb_dyna_processor::update_settings()
        {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                {
                    fp.fFreq        = meta::mb_dyna_processor::FREQ_BOOST_MIN;
                    fp.fFreq2       = 0.0f;
                    fp.fGain        = 1.0f;
                    fp.fQuality     = 0.0f;
                    ...
                    switch (env_boost)
                    {
                        case meta::mb_dyna_processor::FB_BT_3DB:
                            fp.nType        = dspu::FLT_BT_RLC_ENVELOPE;
                            fp.nSlope       = 1;
                            break;
                        case meta::mb_dyna_processor::FB_MT_3DB:
                            fp.nType        = dspu::FLT_MT_RLC_ENVELOPE;
                            fp.nSlope       = 1;
                            break;
                        case meta::mb_dyna_processor::FB_BT_6DB:
                            fp.nType        = dspu::FLT_BT_RLC_ENVELOPE;
                            fp.nSlope       = 2;
                            break;
                        case meta::mb_dyna_processor::FB_MT_6DB:
                            fp.nType        = dspu::FLT_MT_RLC_ENVELOPE;
                            fp.nSlope       = 2;
                            break;
                        case meta::mb_dyna_processor::FB_OFF:
                        default:
            ...
            if (sAnalyzer.needs_reconfiguration())
            {
                ...
                sAnalyzer.get_frequencies(vFreqs, vIndexes, SPEC_FREQ_MIN, SPEC_FREQ_MAX, meta::mb_dyna_processor::MESH_POINTS);
            }
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    b->sProc.set_attack_time(0, b->pAttackTime[0]->value());
                    b->sProc.set_release_time(0, b->pReleaseTime[0]->value());
                    b->sProc.set_hold(b->pHold->value());

                    for (size_t j=0; j<meta::mb_dyna_processor::DOTS; ++j)
                    {
                        b->sProc.set_attack_level(j, (b->pAttackOn[j]->value() >= 0.5f) ? b->pAttackLvl[j]->value() : -1.0f);
                        b->sProc.set_attack_time(j+1, b->pAttackTime[j+1]->value());

                        b->sProc.set_release_level(j, (b->pReleaseOn[j]->value() >= 0.5f) ? b->pReleaseLvl[j]->value() : -1.0f);
                        b->sProc.set_release_time(j+1, b->pReleaseTime[j+1]->value());

                        if ((b->pDotOn[j] != NULL) && (b->pDotOn[j]->value() >= 0.5f))
                            b->sProc.set_dot(j, b->pThreshold[j]->value(), b->pGain[j]->value(), b->pKnee[j]->value());
                        else
                            b->sProc.set_dot(j, -1.0f, -1.0f, -1.0f);
                    }

                    float makeup    = b->pMakeup->value();
                    b->sProc.set_in_ratio(b->pLowRatio->value());
                    b->sProc.set_out_ratio(b->pHighRatio->value());
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (c->nPlanSize <= 0)
                {
                    ...
                    for (size_t j=0; j<c->nPlanSize; ++j)
                    {
                        ...
                        b->sEQ[0].freq_chart(b->vSc, vFreqs, meta::mb_dyna_processor::FFT_MESH_POINTS);
                        dsp::pcomplex_mod(b->vSc, b->vSc, meta::mb_dyna_processor::FFT_MESH_POINTS);
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX-1; ++j)
                {
                    dyna_band_t *b  = (j < (c->nPlanSize-1)) ? c->vPlan[j] : NULL;
        ...
        size_t mb_dyna_processor::select_fft_rank(size_t sample_rate)
        {
        ...
        void mb_dyna_processor::update_sample_rate(long sr)
        {
            ...
            size_t max_delay    = bins + dspu::millis_to_samples(sr, meta::mb_dyna_processor::LOOKAHEAD_MAX);
            ...
            sAnalyzer.init(
                2*channels,
                meta::mb_dyna_processor::FFT_RANK,
                MAX_SAMPLE_RATE,
                meta::mb_dyna_processor::REFRESH_RATE,
                bins);
            sAnalyzer.set_sample_rate(sr);
            sAnalyzer.set_rank(meta::mb_dyna_processor::FFT_RANK);
            sAnalyzer.set_activity(false);
        ...
        void mb_dyna_processor::process_band(void *object, void *subject, size_t band, const float *data, size_t sample, size_t count)
        {
            channel_t *c            = static_cast<channel_t *>(subject);
            dyna_band_t *b          = &c->vBands[band];

            // Store data to band's buffer
            dsp::copy(&b->vBuffer[sample], data, count);
        }

        void mb_dyna_processor::process_input_mono(float *out, const float *in, size_t count)
        {
            if (in != NULL)
        ...
        void mb_dyna_processor::process_input_stereo(float *l_out, float *r_out, const float *l_in, const float *r_in, size_t count)
        {
            if (nMode == MBDP_MS)
            {
                if (l_in != NULL)
        ...
        const float *mb_dyna_processor::select_buffer(const dyna_band_t *band, const channel_t *channel)
        {
            switch (band->nScType)
            {
        ...
        void mb_dyna_processor::preprocess_channel_input(size_t count)
        {
            const size_t channels     = (nMode == MBDP_MONO) ? 1 : 2;
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (sCounter.fired())
                {
                    if (enXOver == XOVER_MODERN)
                    {
                        ...
                        dyna_band_t *b      = c->vPlan[0];
                        sFilters.freq_chart(b->nFilterID, c->vTr, vFreqs, b->fGainLevel, meta::mb_dyna_processor::FFT_MESH_POINTS);
                ...
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    ...
                    if (b->nSync & S_EQ_CURVE)
                    {
                        ...
                        if ((mesh != NULL) && (mesh->isEmpty()))
                        {
                            ...
                            // Fill mesh
                            dsp::copy(&x[2], vFreqs, meta::mb_dyna_processor::MESH_POINTS);
                            dsp::mul_k3(&y[2], b->vSc, b->fScPreamp, meta::mb_dyna_processor::MESH_POINTS);
                            ...
                            y[0]    = 0.0f;
                            y[1]    = y[2];
                            x      += meta::mb_dyna_processor::MESH_POINTS + 2;
                            y      += meta::mb_dyna_processor::MESH_POINTS + 2;
                            ...
                            mesh->data(2, meta::mb_dyna_processor::MESH_POINTS + 4);
        ...
        bool mb_dyna_processor::inline_display(plug::ICanvas *cv, size_t width, size_t height)
        {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                {
                    size_t k        = (j*meta::mb_dyna_processor::MESH_POINTS)/width;
                    b->v[0][j+1]    = vFreqs[k];
        ...
        void mb_dyna_processor::dump(dspu::IStateDumper *v) const
        {
            plug::Module::dump(v);
                ...
                for (size_t i=0; i<channels; ++i)
                {
                    ...
                    for (size_t i=0; i<meta::mb_dyna_processor::BANDS_MAX; ++i)
                    {
                        ...
                        v->begin_object(b, sizeof(dyna_band_t));
                        {
                            ...
                            v->writev("pDotOn", b->pDotOn, meta::mb_dyna_processor::DOTS);
                            v->writev("pThreshold", b->pThreshold, meta::mb_dyna_processor::DOTS);
                            v->writev("pGain", b->pGain, meta::mb_dyna_processor::DOTS);
                            v->writev("pKnee", b->pKnee, meta::mb_dyna_processor::DOTS);
                            v->writev("pAttackOn", b->pAttackOn, meta::mb_dyna_processor::DOTS);
                            v->writev("pAttackLvl", b->pAttackLvl, meta::mb_dyna_processor::DOTS);
                            v->writev("pAttackTime", b->pAttackTime, meta::mb_dyna_processor::RANGES);
                            v->writev("pReleaseOn", b->pReleaseOn, meta::mb_dyna_processor::DOTS);
                            v->writev("pReleaseLvl", b->pReleaseLvl, meta::mb_dyna_processor::DOTS);
                            v->writev("pReleaseTime", b->pReleaseTime, meta::mb_dyna_processor::RANGES);
                            v->write("pHold", b->pHold);
                            v->write("pLowRatio", b->pLowRatio);
                            v->write("pHighRatio", b->pHighRatio);
                    ...
                    v->begin_array("vSplit", c->vBands, meta::mb_dyna_processor::BANDS_MAX-1);
                    for (size_t i=0; i<(meta::mb_dyna_processor::BANDS_MAX-1); ++i)
                    {
                    ...
                    v->end_array();
                    v->writev("vPlan", c->vPlan, meta::mb_dyna_processor::BANDS_MAX);
