...
namespace lsp
{
    namespace plugins
    {
        ...
        static constexpr size_t MBDP_BUFFER_SIZE        = 0x200;
        ...
        void mb_dyna_processor::do_destroy()
        {
            ...
            if (vChannels != NULL)
            {
                for (size_t i=0; i<channels; ++i)
                {
                    ...
                    {
                        dyna_band_t * const b   = &c->vBands[i];
        ...
        void mb_dyna_processor::init(plug::IWrapper *wrapper, plug::IPort **ports)
        {
            ...
            const size_t filter_mesh_size   = align_size(meta::mb_dyna_processor::FFT_MESH_POINTS * sizeof(float), DEFAULT_ALIGN);
            const size_t buf_size           = MBDP_BUFFER_SIZE * sizeof(float);
            ...
            const size_t to_alloc =
                ...
                meta::mb_dyna_processor::CURVE_MESH_SIZE * sizeof(float) + // vCurve
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (!c->sXOver.init(meta::mb_dyna_processor::BANDS_MAX, MBDP_BUFFER_SIZE))
                    return;
            ...
            for (size_t i=0; i<channels; ++i)
            {
                for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX; ++j)
                {
                    dyna_band_t * const b   = &vChannels[i].vBands[j];

                    if ((i > 0) && (nMode == MBDP_STEREO))
                    {
                        dyna_band_t * const sb  = &vChannels[0].vBands[j];
            ...
            const float delta = (meta::mb_dyna_processor::CURVE_DB_MAX - meta::mb_dyna_processor::CURVE_DB_MIN) / (meta::mb_dyna_processor::CURVE_MESH_SIZE-1);
        ...
        void mb_dyna_processor::update_settings()
        {
            ...
            for (size_t i=0; i<channels; ++i)
            {
                ...
                if (c->nPlanSize <= 0)
                {
                    ...
                    for (size_t j=0; j<meta::mb_dyna_processor::BANDS_MAX-1; ++j)
                    {
                        dyna_band_t * const b   = &c->vBands[j+1];
        ...
        void mb_dyna_processor::process_band(void *object, void *subject, size_t band, const float *data, size_t sample, size_t count)
        {
            ...
            dyna_band_t * const b   = &c->vBands[band];
                ...
                if (sCounter.fired())
                {
                    ...
                    else if (enXOver == XOVER_CLASSIC)
                    {
                        ...
                        for (size_t j=0; j<c->nPlanSize; ++j)
                        {
                            ...
                            {
                                c->sXOver.freq_chart(band, b->vTr, vFreqs, meta::mb_dyna_processor::FFT_MESH_POINTS);
                                b->nSync           &= ~size_t(S_BAND_CURVE);
                            }
                            if (j == 0)
                                dsp::mul_k3(c->vTr, b->vTr, b->fGainLevel, meta::mb_dyna_processor::FFT_MESH_POINTS*2);
                            else
                                dsp::fmadd_k3(c->vTr, b->vTr, b->fGainLevel, meta::mb_dyna_processor::FFT_MESH_POINTS*2);
                        }
                        dsp::pcomplex_mod(c->vTrMem, c->vTr, meta::mb_dyna_processor::FFT_MESH_POINTS);
